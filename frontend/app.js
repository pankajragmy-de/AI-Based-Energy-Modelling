...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function displayResults(framework, result) {
        ...
        let attrHtml = "";
        if(framework === "PyPSA") {
            attrHtml = `Generated by <a href="#" class="text-white font-bold underline">PyPSA</a> via TU Berlin. Licensed under MIT. Computed 8760 hourly periods dynamically.`;
        } else if(framework === "OEMOF") {
            attrHtml = `Generated by <a href="#" class="text-white font-bold underline">OEMOF</a> via RLI. Licensed under MIT. Computed 8760 hourly periods dynamically.`;
        } else {
             attrHtml = `Generated by ${framework} engine. Computed 8760 hourly periods dynamically.`;
        }
        document.getElementById('res-attribution-text').innerHTML = attrHtml;
        
        renderChart(result.generation_mix);
        renderDispatchChart(result.timeseries);
    }
        ...
        dispatchChartInstance = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [
                    ...
                    {
                        label: 'Storage Dispatch / Chg (MW)',
                        data: ts.storage_dispatch,
            ...
            options: {
                ...
                scales: {
                    x: { ticks: { color: '#6B7280', maxTicksLimit: 12 }, grid: { color: 'rgba(75, 85, 99, 0.2)' } },
                    y: { title: { display: true, text: 'Megawatts (MW)', color: '#6B7280' }, ticks: { color: '#6B7280' }, grid: { color: 'rgba(75, 85, 99, 0.2)' } }
                },
                plugins: {
